...
export default function Upload() {
  ...
  return (
    <div className={`bg-transparent flex flex-col items-center w-full p-4 text-[#ffffff] fixed bottom-0 ${files.length === 0 ? "hidden" : ""}`}>
      <Dialog>
        ...
        <DialogContent className="sm:max-w-[425px]">
          {clickedupload ? (
            // Show only the progress bar when upload is in progress
            <>
              <div className="w-full mt-6 max-w-lg bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                <div
                  className="bg-blue-600 h-2.5 rounded-full"
                  style={{ width: `${overallProgress}%` }}
                ></div>
              </div>
              <div className="text-black mt-2 flex items-center justify-between">
                <span>{`Progress: ${overallProgress.toFixed(2)}%`}</span>
              </div>
            </>
          ) : (
            // Show the password setup options when not uploading
            <>
              <DialogHeader>
